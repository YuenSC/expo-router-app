...
export const getExpensePaymentRelationship = async ({
  groupId,
}: GetExpensePaymentRelationshipPayload): Promise<GetExpensePaymentRelationshipResponse> => {
  return (
    await axios.get(`/api/groups/${groupId}/expenses/payment-relationship`)
  ).data;
};
